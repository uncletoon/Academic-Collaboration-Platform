...
const statusStyles = {
  ...
  active: "portal-badge portal-badge--joined",
  completed: "portal-badge portal-badge--done",
  archived: "portal-badge",
};
...
const ProjectsPage = () => {
  ...
  const upload = async (event) => {
    ...
    body.append("projectFile", file);
    try {
      await api.uploadProjectFile(selected.id, body);
  ...
  const filtered = useMemo(
    () =>
      projects.filter((project) => {
        const found = `${project.title} ${project.description}`
          .toLowerCase()
  ...
  const groupedProjects = useMemo(
    () => ({
      created: filtered.filter(
        (project) => Number(project.created_by) === Number(user?.id),
      ),
  ...
  const renderProjectCard = (project) => (
    <button
      key={project.id}
      className="portal-resource-card portal-resource-card--project"
    ...
    >
      <span className="portal-resource-card__head">
        ...
        <span className={statusStyles[project.status] || "portal-badge"}>
          {project.status}
        </span>
      </span>
      ...
      <span className="portal-resource-card__footer">
        <span>
          <Users /> {project.member_count || 1} collaborators
        </span>
        <span>
          <Workflow /> {project.user_role || "lead"}
        </span>
      </span>
    </button>
  );
          ...
          <div>
            ...
            <p>
              {selected.description ||
                "This project has not added a description yet."}
            </p>
            <div className="portal-detail-meta">
              <span>
                <Workflow /> {selected.status}
              </span>
              <span>
                <Users /> {members.length} collaborators
              </span>
              <span>
                <CalendarClock /> Started{" "}
                {new Date(selected.created_at).toLocaleDateString()}
              </span>
            </div>
          </div>
          <span className={statusStyles[selected.status] || "portal-badge"}>
            {selected.status}
          </span>
        </section>
        ...
        <div className="portal-detail-grid">
          <main className="portal-panel">
            <div className="portal-panel__head">
              <div>
                <p className="portal-eyebrow">Shared assets</p>
                <h2>Project files</h2>
              </div>
            ...
            {files.length ? (
                ...
                {files.map((file) => (
                  ...
                  >
                    ...
                    <span>
                      ...
                      <small>
                        Added by {file.uploaded_by_name || "a collaborator"} ·{" "}
              ...
              <EmptyState
                ...
                description="Upload a brief, dataset, notes, or another project resource."
              />
            )}
          </main>
            ...
            {members.map((member) => (
              <div key={member.id} className="portal-person">
                ...
                <span>
                  ...
                  <small>
                    {member.project_role} · {member.user_role}
                  </small>
                </span>
              </div>
            ))}
  ...
  return (
    <div className="portal-page space-y-8 pb-16">
      <PageIntro
        ...
        title={
          <>
            Give promising ideas a place to <em>become real.</em>
          </>
        }
        action={
          <button
      ...
      >
        <div className="portal-hero__stats">
          <StatPill
            ...
            tone="cyan"
          />
          <StatPill icon={Users} value={joinedCount} label="Joined projects" />
          <StatPill
            icon={Workflow}
      ...
      <section>
        <div className="portal-toolbar">
          <SearchField
            value={search}
            onChange={setSearch}
            placeholder="Search your project workspaces"
        ...
        {error && <div className="portal-alert">{error}</div>}
        {loading ? (
          <div className="portal-loading">Opening your workspaces…</div>
        ) : filtered.length ? (
          <div className="portal-community-groups">
            <section
              className="portal-community-group"
              aria-labelledby="created-projects-heading"
            >
              <div className="portal-group-heading">
                <div>
                  <p className="portal-eyebrow">Led by you</p>
                  <h2 id="created-projects-heading">My projects</h2>
                </div>
                <span>{groupedProjects.created.length}</span>
              </div>
              {groupedProjects.created.length ? (
            ...
            >
              <div className="portal-group-heading">
                <div>
                  <p className="portal-eyebrow">Team workspaces</p>
                  <h2 id="joined-projects-heading">Joined projects</h2>
                </div>
                <span>{groupedProjects.joined.length}</span>
              </div>
              {groupedProjects.joined.length ? (
                ...
                <div className="portal-group-empty">
                  Projects where you collaborate will appear here.
                </div>
              )}
            </section>
            <section
            ...
            >
              <div className="portal-group-heading">
                <div>
                  <p className="portal-eyebrow">Finished work</p>
                  <h2 id="completed-projects-heading">
                    Completed collaborations
                  </h2>
                </div>
                <span>{groupedProjects.completed.length}</span>
              </div>
              {groupedProjects.completed.length ? (
                ...
                <div className="portal-group-empty">
                  Completed collaborations will be kept here.
                </div>
              )}
            </section>
          </div>
          ...
          <EmptyState
            ...
            description="Adjust the search or start a project for your next research direction."
          />
        )}
      </section>
      {showCreate && (
        <Modal
          ...
          description="Define the shared purpose first; collaborators and files can follow."
          ...
          <form className="portal-form" onSubmit={create}>
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="What question will the team answer, and what outcome are you working toward?"
              />
            </label>
